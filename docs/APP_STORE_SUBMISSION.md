...
```sh
./scripts/build.sh                       # build/Quiet.app を生成
# Developer ID で署名し直す（ad-hoc を上書き）:
codesign --force --deep --options runtime \
  --sign "Developer ID Application: <Your Name> (<TEAMID>)" \
  --entitlements Pomo.entitlements build/Quiet.app
# 公証:
ditto -c -k --keepParent build/Quiet.app Quiet.zip
xcrun notarytool submit Quiet.zip --apple-id <id> --team-id <TEAMID> --password <app専用pw> --wait
xcrun stapler staple build/Quiet.app
```
...
```
Xcode で:
1. `Pomo` ターゲット（Xcode 上のターゲット名は `Sources/Pomo` に合わせて据え置き。生成物は `PRODUCT_NAME: Quiet` により Quiet.app）→ Signing & Capabilities → Team を選択（自動署名）。`project.yml` の `DEVELOPMENT_TEAM` に Team ID を入れてもよい。
...
Claude Code は GUI を確認できない。`./scripts/build.sh && open build/Quiet.app` で起動し、以下を目視:
