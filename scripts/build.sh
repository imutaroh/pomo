#!/bin/zsh
# Quiet を release ビルドして Quiet.app に束ね、ad-hoc 署名する（REQUIREMENTS.md §9 配布方針）
# SwiftPM のターゲット名・bundle id は pomo 時代のまま（Sparkle の同一性維持）。成果物名だけ Quiet
set -euo pipefail
cd "$(dirname "$0")/.."
...
cp .build/release/Pomo "$APP/Contents/MacOS/Quiet"
...
install_name_tool -add_rpath "@loader_path/../Frameworks" "$APP/Contents/MacOS/Quiet" 2>/dev/null || true
...
<dict>
    ...
    <key>CFBundleExecutable</key><string>Quiet</string>
    <key>CFBundleIdentifier</key><string>com.imutaakihiro.pomo</string>
    <key>CFBundleName</key><string>Quiet</string>
    <key>CFBundleDisplayName</key><string>Quiet</string>
